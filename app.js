...
function updateCurrentLyric() {
  ...
  for (let i = 0; i < currentLyrics.length; i++) {
    if (
      currentLyrics[i].startTime <= currentTime &&
      (i === currentLyrics.length - 1 ||
        currentLyrics[i + 1].startTime > currentTime)
    ) {
      currentIndex = i;
      break;
    }
...
function updateNavState() {
  ...
  navVideo.className =
    "px-2 py-1 transition-all duration-200 rounded font-medium text-gray-500 opacity-60 cursor-pointer";
  ...
  if (videoLoaded) {
    ...
    navLyrics.className =
      "px-2 py-1 transition-all duration-200 rounded font-medium text-gray-400 opacity-40 cursor-default";
  }
...
function goToStep(step) {
  ...
  switch (step) {
    case "video":
      ...
      navVideo.className =
        "px-2 py-1 transition-all duration-200 rounded font-semibold text-blue-500 opacity-100 cursor-pointer";
...
function displayLyricsWithPinyin(lyrics) {
  ...
  lyrics.forEach((line, index) => {
    ...
    lineElement.className =
      "px-2 py-1 mb-2 rounded cursor-pointer transition-all duration-200 border-l-4 border-transparent animate-fade-in relative";

    const pinyinElement = document.createElement("div");
    pinyinElement.className =
      "text-base text-gray-500 dark:text-gray-400 leading-snug transition-colors duration-300";
    ...
    chineseElement.className =
      "text-lg font-medium leading-relaxed tracking-widest text-gray-800 dark:text-gray-200 transition-colors duration-300";
...
function displaySearchResults(results, cleanedQuery = null) {
  ...
  if (results.length === 0) {
    searchResults.innerHTML =
      '<p class="text-gray-500 dark:text-gray-400 p-4 text-center text-sm transition-colors duration-300">No lyrics found. Try "Artist - Song" for best results.</p>';
    return;
  }
  ...
  headerElement.className =
    "text-xs text-gray-500 dark:text-gray-400 px-2 py-1 bg-gray-50 dark:bg-gray-700 transition-colors duration-300";
  ...
  results.forEach((result) => {
    ...
    resultElement.className =
      "p-2 border-b border-gray-200 dark:border-gray-700 cursor-pointer animate-fade-in hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-300";
    ...
    resultElement.addEventListener("click", async () => {
      ...
      lyricsView.innerHTML =
        '<div class="flex justify-center p-4"><div class="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500 dark:border-blue-400"></div></div>';
      ...
      try {
        ...
        lyricsView.innerHTML =
          '<p class="text-red-500 dark:text-red-400 text-center p-4 text-sm transition-colors duration-300">Error loading lyrics. Please try again.</p>';
        goToStep("lyrics");
      }
    });
...
async function loadVideo() {
  ...
  videoInput.innerHTML =
    '<div class="flex justify-center p-4"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div><div class="ml-3">Loading video...</div></div>';
  ...
  try {
    ...
    if (results.length === 0 && !isRetry) {
      ...
      if (cleanedQuery !== query && cleanedQuery.length > 0) {
        console.log(
          `No results found. Retrying with cleaned query: "${cleanedQuery}"`
        );
    ...
    searchResults.innerHTML =
      '<p class="text-red-500 dark:text-red-400 p-4 text-center text-sm transition-colors duration-300">Error searching. Please try again.</p>';
  }
}, 500);
...
navPlayer.addEventListener("click", () => {
  ...
  if (
    !navPlayer.classList.contains("disabled") &&
    player &&
